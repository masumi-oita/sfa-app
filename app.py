...
(Integrated: All v1.4.7 Features + Dynamic Filtering & Deep Drilldown)
"""
...
def set_page() -> None:
    ...
    st.title(APP_TITLE)
    st.caption("OS v1.5.1｜全機能統合版（グループ絞り込み × 採用アラート × 実態解明）")
...
def render_ranking(client, where_clause, params):
    ...
    sql = f"""
        WITH base AS (
            SELECT COALESCE(NULLIF(CAST(yj_code AS STRING), "0"), CAST(jan_code AS STRING)) AS code, MAX(product_name) AS name,
            SUM(CASE WHEN fiscal_year = 2025 THEN sales_amount ELSE 0 END) AS ty, SUM(CASE WHEN fiscal_year = 2024 THEN sales_amount ELSE 0 END) AS py
            FROM `{VIEW_UNIFIED}` {where_clause} GROUP BY 1
        )
        SELECT code AS `コード`, name AS `商品名`, ty AS `今期`, py AS `前期`, (ty - py) AS `差額`
        FROM base WHERE py > 0 AND (ty - py) != 0 ORDER BY `差額` {sort_dir} LIMIT 50
    """
    ...
    if not df.empty:
        ...
        if sel_code:
            ...
            sql_jan = f"""
                ...
                FROM `{VIEW_UNIFIED}` {where_clause} AND COALESCE(NULLIF(CAST(yj_code AS STRING), "0"), CAST(jan_code AS STRING)) = @code
            ...
            sql_cust = f"""
                SELECT customer_name AS `得意先`, SUM(CASE WHEN fiscal_year = 2025 THEN sales_amount ELSE 0 END) AS `今期`,
...
def render_new_deliveries(client, where_clause, params):
    ...
    # ※注: 新規納品ビューにも sales_group_name があれば where_clause が使えます
    # なければ全社表示になります
    try:
        sql = f"""
        ...
        if not df.empty: st.dataframe(df.style.format({"売上": "¥{:,.0f}"}), use_container_width=True, hide_index=True)
    except: st.empty()

def render_adoption_alerts(client, where_clause, params):
    """採用アラート（フィルター連動は限定的）"""
    st.subheader("🚨 採用アイテム・失注アラート")
    # ※注: v_customer_adoption_status にも group列 があれば連動可能。今回は簡易実装
    try:
        sql = f"SELECT staff_name, customer_name, product_name, last_purchase_date, adoption_status, current_fy_sales, previous_fy_sales FROM `{VIEW_ADOPTION}` ORDER BY 5, 6 ASC LIMIT 100"
