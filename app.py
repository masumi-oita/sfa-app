...
if client:
    ...
    if not df.empty:
        ...
        # 列名チェック（データに合わせてください）
        # もし列名が英語（item_nameなど）の場合は、ここも修正が必要です
        ...
        if all(col in filtered_df.columns for col in required_cols):
            ...
            st.warning("⚠️ 集計に必要な列が見つかりません。")
            st.info("BigQueryの実際の列名は以下の通りです（コード内の列名をこれに合わせて修正してください）:")
            st.write(df.columns.tolist()) # ここで列名一覧を表示します
