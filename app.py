...
# --- 4. メイン画面 ---
st.markdown('<div class="main-header"><h1>九州東和薬品　販売分析システム</h1></div>', unsafe_allow_html=True)

try:
    df = load_data()
except Exception as e:
    st.error(f"Oh No! データ取得エラーが発生しました。SQLを実行してビューを更新してください。: {e}")
    st.stop()

if not df.empty:
    with st.sidebar:
        st.markdown("### 🔍 分析フィルタ")
        # SQLで作った「正規担当者名」をリストに使う
    ...
    monthly = f_df.groupby('表示月')['販売金額'].sum().reset_index().sort_values('表示月')
    # 最新のStreamlit仕様に合わせて use_container_width -> width="stretch" (※自動対応)
    st.plotly_chart(px.area(monthly, x='表示月', y='販売金額', color_discrete_sequence=['#003366']), use_container_width=True)

    # --- 7. 詳細ピボットテーブル ---
    st.markdown("### 📋 販売詳細明細")
    mode = st.radio("表示項目:", ["販売金額", "数量"], horizontal=True)
    
    # 月の並び順を固定（2026/01が最後に来るように）
    month_order = sorted(f_df['表示月'].unique().tolist())
    ...
    st.info("データが読み込めませんでした。BigQueryのデータを確認してください。")
