...
def load_snapshot_2y():
    ...
    sales_history_2year:
    - 販売日がSTRING想定
    - 当月は後続で捨てるが、SQL側で当月以前に絞って転送量を減らす
    """
...
def load_incremental_this_month():
    """
    sales_details_snapshots:
    - 当月分は洗い替え（GAS側で月次DELETE→APPEND）
    - 当月だけ取得して軽量化
    """
    client = get_bq_client()
    q = f"""
...
def add_new_delivery_flag_by_unique_yj(df: pd.DataFrame, lookback_days=365) -> pd.DataFrame:
    d = df.copy()
    d = d.sort_values(["ユニークコード_YJ", "売上日"])
...
c2.metric("統合（重複排除済）", f"{len(df_sales):,}")
...
# =========================
# ① FY-to-date vs last-year-to-date (Customer rank)
# =========================
...
with st.expander("得意先→品目ドリルダウン"):
    cust_sum = new_df.groupby("得意先名")["売上額"].sum().sort_values(ascending=False).reset_index()
    sel = st.selectbox("得意先", ["-- 選択 --"] + cust_sum["得意先名"].tolist())
    if sel != "-- 選択 --":
        d = new_df[new_df["得意先名"] == sel].copy()
        d2 = d.groupby(["YJコード","商品名"], dropna=False).agg(売上=("売上額","sum"), 利益=("利益","sum")).reset_index()
        d2["利益率"] = d2.apply(lambda r: (r["利益"]/r["売上"]) if r["売上"] else 0, axis=1)
        d2 = d2.sort_values("売上", ascending=False)
        st.dataframe(d2.style.format({"売上":"¥{:,.0f}","利益":"¥{:,.0f}","利益率":"{:.2%}"}), use_container_width=True)

# =========================
# ③ Lost / Growth (customer diff)
# =========================
st.divider()
st.header("③ 下降 / 上昇 得意先差額ランキング（年度内 vs 昨年同日まで）")

# customer-year totals
...
with tab_l:
    loss = cd.sort_values("差額", ascending=True).head(30)
    st.dataframe(loss.style.format({"売上_今年":"¥{:,.0f}","売上_昨年":"¥{:,.0f}","差額":"¥{:,.0f}"}), use_container_width=True)

with tab_g:
    gain = cd.sort_values("差額", ascending=False).head(30)
    st.dataframe(gain.style.format({"売上_今年":"¥{:,.0f}","売上_昨年":"¥{:,.0f}","差額":"¥{:,.0f}"}), use_container_width=True)

st.caption("当月は sales_details_snapshots を採用（洗い替え）。sales_history_2year の当月は除外して重複を防止。")
